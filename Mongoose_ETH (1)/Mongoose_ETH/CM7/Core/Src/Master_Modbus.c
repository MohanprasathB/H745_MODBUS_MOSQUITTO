...
void ModbusMaster_SendRequest(unsigned char slave_id, unsigned char function_code, unsigned short start_address, unsigned short data_length, unsigned short *data)
{
    ...
    ModbusMaster_outbox[ModbusMaster_Tx_index++] = (start_address >> 8) & 0xFF;//MSB
    ModbusMaster_outbox[ModbusMaster_Tx_index++] = start_address & 0xFF;//LSB
    ModbusMaster_outbox[ModbusMaster_Tx_index++] = (data_length >> 8) & 0xFF; //MSB
    ModbusMaster_outbox[ModbusMaster_Tx_index++] = data_length & 0xFF; //LSB

    /* Calculate CRC */
    unsigned short crc = ModbusMaster_CalculateCRC(ModbusMaster_outbox, ModbusMaster_Tx_index);
    ModbusMaster_outbox[ModbusMaster_Tx_index++] = crc & 0xFF; //LSB
    ModbusMaster_outbox[ModbusMaster_Tx_index++] = (crc >> 8) & 0xFF; //MSB

    // Debug
    printf("\rModbus Request: ");
...
unsigned short ModbusMaster_ReceiveResponse(unsigned char *buffer, unsigned short expected_len)
{
	 if(HAL_UART_Receive(&huart2, buffer, expected_len, MODBUS_MASTER_TIMEOUT_MS) == HAL_OK) {
	    // CRC check
	    unsigned short crc_received = (buffer[expected_len-1] << 8) | buffer[expected_len-2];
	    unsigned short crc_calculated = ModbusMaster_CalculateCRC(buffer, expected_len-2);
